...
Model* modelForSecondList = new Model;

View* firstView = new View(modelForFirstList, 105);
Controller firstController(modelForFirstList, firstView);

View* secondView = new View(modelForSecondList, 106);
Controller secondController(modelForSecondList, secondView);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg,  WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
		...
		case WM_COMMAND:
			if (reinterpret_cast<HWND>(lParam) == firstPushBackButton) {
				firstController.pushBack(firstFieldWithNumberToPush);
			}
			else if (reinterpret_cast<HWND>(lParam) == firstPopBackButton)
				firstController.popBack();
			else if (reinterpret_cast<HWND>(lParam) == firstPushFrontButton) {
				firstController.pushFront(firstFieldWithNumberToPush);
			}
				...
				secondController.pushBack(secondFieldWithNumberToPush);
			}
				...
				secondController.pushFront(secondFieldWithNumberToPush);
			}
				...
				firstController.swap(secondController);
				...
				SendDlgItemMessage(hWnd, 107, LB_INSERTSTRING, -1, (LPARAM)std::to_string(visitor.getAverage()).data());
			}
			else if (reinterpret_cast<HWND>(lParam) == secondAverageButton) {
				firstController.accept(visitor);
				SendDlgItemMessage(hWnd, 108, LB_RESETCONTENT, NULL, NULL);
				SendDlgItemMessage(hWnd, 108, LB_INSERTSTRING, -1, (LPARAM)std::to_string(visitor.getAverage()).data());
			}
			else
